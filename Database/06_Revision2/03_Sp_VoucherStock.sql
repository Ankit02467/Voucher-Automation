/* ============================================================
   View Data proc - revision 2.

   - Dealers come from VoucherDealer_Table as pipe separated lists,
     so the screen can show any number of dealer columns.
   - Fresh uploads carry no status (NULL) until someone sets one.
   - Move / reassign flow between student and sub-admin, all logged
     to VoucherHistory_Table.
   ============================================================ */
USE DSL_New;
GO
...
(
    ...
    @IsMoved          NVARCHAR(5)    = NULL,   -- '1' = only moved rows, '0' = only open rows
...
BEGIN
    ...
    BEGIN
        ...
        SELECT
            ...
            /* dealer 1..n, pipe separated and index aligned */
            DealerNames = ISNULL((
    ...
    /* largest number of dealer slots in use - drives the grid columns */
    ...
    BEGIN
        ...
        /* Status stays NULL - a fresh entry has no status yet */
    ...
    ELSE IF @Action = 'Move'
    BEGIN
        /* only a checked voucher that already has a status can move on */
        UPDATE dbo.VoucherStock_Table
